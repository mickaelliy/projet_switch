...
*********************************************************/
  ...
  if(empty($msg)) {
    ...
    $requete = $pdo->prepare("INSERT INTO produit (date_arrivee, date_depart, prix, etat) VALUES (:date_arrivee, :date_depart, :prix, :etat)");
  } else {
    $requete = $pdo->prepare("UPDATE produit SET date_arrivee = :date_arrivee, date_depart = :date_depart, prix = :prix, etat = :etat WHERE id_produit = :id_produit");
        ...
        <?php 
          ...
          if(isset($_GET['action']) && $_GET['action'] == 'afficher') {
            ...
            echo '<div class="col-12">';
              ...
              // Tableau
              $listeproduit = $pdo->query("SELECT p.id_produit, date_format(p.date_arrivee, '%d/%m/%Y') AS 'Date arrivée', date_format(p.date_depart, '%d/%m/%Y') AS 'Date départ', p.id_salle, s.photo, p.prix, p.etat FROM produit p, salle s");//Requête méthode jointure
